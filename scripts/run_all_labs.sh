...
$CMD -S $S1 -Q "CREATE DATABASE [Test] ON PRIMARY (NAME = N'testdata_a', FILENAME = N'/var/opt/mssql/data/testdata_a.mdf', SIZE = 10MB);"
...
$CMD -S $S1 -Q "CREATE LOGIN [TestLogin1] WITH PASSWORD = '$P'; ALTER SERVER ROLE [sysadmin] ADD MEMBER [TestLogin1];"
$CMD -S $S1 -Q "EXEC sp_configure 'show advanced options', 1; RECONFIGURE; EXEC sp_configure 'Database Mail XPs', 1; RECONFIGURE;"
$CMD -S $S1 -Q "EXEC msdb.dbo.sp_add_operator @name = 'DBA_Admin', @enabled = 1, @email_address = '$MY_EMAIL';"
...
$CMD -S $S1 -d ProjectDB -Q "INSERT INTO OrderDetails (ProductID, Quantity) SELECT TOP 1000 1, 10 FROM sys.objects;"
$CMD -S $S1 -d ProjectDB -Q "CREATE COLUMNSTORE INDEX [IX_CS_OrderDetails] ON [OrderDetails] (ProductID, Quantity);"
